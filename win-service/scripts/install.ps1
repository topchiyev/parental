...
function Configure-ServiceRecovery {
  ...
  Write-Log "Configuring recovery actions for service '$Name'."

  # Restart on first/second/subsequent failures.
  # Delays: 5s, 15s, 60s. Reset failure counter after 1 day.
  $failureOutput = & sc.exe failure $Name reset= 86400 actions= restart/5000/restart/15000/restart/60000 2>&1
  $failureExitCode = $LASTEXITCODE
  if ($failureOutput) { $failureOutput | ForEach-Object { Write-Log "sc failure: $_" } }
  if ($failureExitCode -ne 0) {
    throw "Failed to configure service recovery actions (sc.exe failure exit code $failureExitCode)."
  }

  # Ensure recovery actions are also used for non-crash failures.
  $flagOutput = & sc.exe failureflag $Name 1 2>&1
  $flagExitCode = $LASTEXITCODE
  if ($flagOutput) { $flagOutput | ForEach-Object { Write-Log "sc failureflag: $_" } }
  if ($flagExitCode -ne 0) {
    throw "Failed to configure service failure flag (sc.exe failureflag exit code $flagExitCode)."
  }

  Write-Log "Service recovery configured successfully."
}
...
try {
  ...
  Configure-ServiceRecovery -Name $Script:ServiceName
